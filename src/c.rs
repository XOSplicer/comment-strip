...
fn state_transition(from: ParseState, current_char: Option<char>) -> (ParseState, ParseAction) {
    match current_char {
        ...
        None => match from {
            ParseState::FirstSlash => (ParseState::End, ParseAction::CommentDismissed),
            ParseState::SingleLineComment => (ParseState::End, ParseAction::CommentEnds),
            ParseState::MultiLineComment => (ParseState::End, ParseAction::CommentDismissed),
            ParseState::MultiLineCommentFinalStar => {
                (ParseState::End, ParseAction::CommentDismissed)
            }
            ParseState::MultiLineCommentFinalSlash => (ParseState::End, ParseAction::CommentEnds),
...
fn do_action(
    ...
    match action {
        ...
        ParseAction::CommentDismissed => comment_state = CommentState::NotInComment,
